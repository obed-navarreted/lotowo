...
import {
  LimitDrawType, RouteLimitSeller, RouteNumberLimitPolicy, RouteNumberLimits
...
export class RouteLimitsPage implements OnInit {
  ...
  protected readonly drafts = signal<Record<LimitDrawType, RouteLimitDraft>>({
    DAILY: this.emptyDraft(), NATIONAL_LOTTERY: this.emptyDraft()
  });
  protected readonly current = computed(() => this.drafts()[this.selectedType()]);

  ngOnInit(): void {
    this.api.getRouteNumberLimits(this.routeId).pipe(finalize(() => this.loading.set(false))).subscribe({
      next: (response) => this.applyResponse(response),
      error: (apiError: unknown) => this.error.set(apiErrorMessage(apiError, 'No fue posible cargar los límites de la ruta.'))
    });
  }
  ...
  protected setScope(appliesToAll: boolean): void {
    this.updateCurrent((draft) => ({
      ...draft,
      appliesToAll,
      sellerIds: appliesToAll ? new Set(this.sellers().map((seller) => seller.id)) : new Set(draft.sellerIds)
    }));
  }
  ...
  protected toggleSeller(sellerId: string, checked: boolean): void {
    this.updateCurrent((draft) => {
      ...
      if (checked) sellerIds.add(sellerId); else sellerIds.delete(sellerId);
      return { ...draft, sellerIds };
    });
  }

  protected selectAllSellers(): void {
    this.updateCurrent((draft) => ({ ...draft, sellerIds: new Set(this.sellers().map((seller) => seller.id)) }));
  }
  ...
  protected isOverride(number: string): boolean { return this.current().overrides.has(number); }
  protected isBlocked(number: string): boolean { return this.effectiveLimit(number) === 0; }

  protected save(): void {
    const draft = this.current();
    if (!draft.appliesToAll && draft.sellerIds.size === 0) {
    ...
    this.api.updateRouteNumberLimits(this.routeId, this.selectedType(), {
      ...
      next: (response) => {
        ...
        this.notice.set(`Los límites de ${this.typeLabel(this.selectedType())} se aplicaron correctamente.`);
      },
      error: (apiError: unknown) => this.error.set(apiErrorMessage(apiError, 'No fue posible guardar los límites de la ruta.'))
    });
  }
  ...
  private emptyDraft(sellers: RouteLimitSeller[] = []): RouteLimitDraft {
    return { defaultLimit: null, overrides: new Map(), appliesToAll: true, sellerIds: new Set(sellers.map((seller) => seller.id)) };
  }
